...
pub fn run(config_path: PathBuf) -> smolvm::Result<()> {
    ...
    // Start the TSI-only DNS filter listener if configured. Virtio enforces
    // DNS policy in the host runtime directly and does not use the guest-side
    // DNS proxy socket.
    let network_plan = smolvm::network::plan_launch_network(
        &config.resources,
        config.dns_filter_hosts.as_deref(),
        config.ports.len(),
    );
    let dns_filter_socket_path = if matches!(
        network_plan.backend,
        smolvm::network::EffectiveNetworkBackend::Tsi
    ) {
        if let Some(ref hosts) = config.dns_filter_hosts {
            if !hosts.is_empty() {
    ...
    let result = launch_agent_vm(&LaunchConfig {
        ...
        dns_filter_hosts: config.dns_filter_hosts.as_deref(),
    });
