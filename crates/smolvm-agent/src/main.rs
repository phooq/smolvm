...
fn handle_interactive_run(
    ...
    if let Err(e) = storage::setup_mounts(&prepared.rootfs_path, &mounts) {
        if let Err(cleanup_err) = storage::cleanup_overlay(&prepared.workload_id) {
            warn!(
                workload_id = %prepared.workload_id,
                error = %cleanup_err,
                "failed to clean up overlay after mount setup failure"
            );
        }
        send_response(
            stream,
        ...
        Err(e) => {
            if let Err(cleanup_err) = storage::cleanup_overlay(&prepared.workload_id) {
                warn!(
                    workload_id = %prepared.workload_id,
                    error = %cleanup_err,
                    "failed to clean up overlay after spawn failure"
                );
            }
            send_response(
                stream,
        ...
        Err(e) => {
            if let Err(cleanup_err) = storage::cleanup_overlay(&prepared.workload_id) {
                warn!(
                    workload_id = %prepared.workload_id,
                    error = %cleanup_err,
                    "failed to clean up overlay after interactive loop error"
                );
            }
            return Err(e);
        }
    ...
    if let Err(cleanup_err) = storage::cleanup_overlay(&prepared.workload_id) {
        warn!(
            workload_id = %prepared.workload_id,
            error = %cleanup_err,
            "failed to clean up overlay after interactive run"
        );
    }
